...
def search_and_save_movie_info(api_key, movie_title):
    ...
    if movie_info:
        for movie in movie_info:
            file_name = f"{movie['title']}.json"  # 파일명을 영화 이름으로 지정
            with open(file_name, "w", encoding="utf-8") as json_file:
                json.dump(movie, json_file, ensure_ascii=False, indent=4)
    else:
        print(f"'{movie_title}'에 대한 영화 정보를 찾을 수 없습니다.")

# 함수를 사용하여 영화 정보 검색 및 저장
search_and_save_movie_info('7c11fe750835c7c0b4c2bb32573c709f', '인터스텔라')
